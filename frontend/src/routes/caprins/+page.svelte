<!-- src/routes/caprins/+page.svelte -->
<script lang="ts">
    ...
    function getStoredFilters(): AnimauxFilters | null {
        const stored = caprinsStore.filters.get();
        if (stored) {
            return stored;
        }
        return null;
    ...
    let filteredCaprins: CaprinResponse[] = [];
    let enclosList: EnclosResponse[] = [];
    let loading = true;
    let showModal = false;
    let selectedCaprin: CaprinResponse | null = null;
    let isEdit = false;
    ...
    const sexeOptions: { value: Sexe; label: string; emoji: string }[] = [
        { value: "male", label: "Bouc", emoji: "♂" },
        { value: "femelle", label: "Chèvre", emoji: "♀" },
    ...
    function updateFilters() {
        ...
        caprinsStore.filters.set(filtersToStore);
        ...
        const url = `/caprins${params.toString() ? '?' + params.toString() : ''}`;
    ...
    async function loadData() {
        ...
        try {
            ...
            const response = await caprinsApi.getCaprins({ 
                limit: 200, 
            ...
            caprins = response.items || response || [];
            stats.total = response.total || caprins.length;
            applyFilters();
            calculateStats();
            updateFilters(); // ✅ Sauvegarder les filtres après chargement
        } catch (error) {
            console.error("Failed to load caprins:", error);
        } finally {
            loading = false;
        }
    }
    ...
    function applyFilters() {
        filteredCaprins = caprins.filter((c) => {
            const matchesSearch =
                !searchQuery ||
                c.identification?.toLowerCase().includes(searchQuery.toLowerCase()) ||
                c.race?.toLowerCase().includes(searchQuery.toLowerCase());

            const matchesRace = !selectedRace || c.race === selectedRace;
            const matchesEnclos = !selectedEnclos || c.enclos_id === Number(selectedEnclos);
            const matchesStatut = selectedStatuts.length === 0 || selectedStatuts.includes(c.statut as StatutAnimal);
            const matchesSexe = selectedSexes.length === 0 || selectedSexes.includes(c.sexe as Sexe);
    ...
    function calculateStats() {
        stats.males = caprins.filter((c) => c.sexe === "male").length;
        stats.femelles = caprins.filter((c) => c.sexe === "femelle").length;
        stats.hermaphrodites = caprins.filter((c) => c.sexe === "hermaphrodite").length;
        stats.productionViande = caprins.filter((c) => c.production_viande).length;
        stats.productionReproduction = caprins.filter((c) => c.production_reproduction).length;
    }
    ...
    function handleEdit(caprin: CaprinResponse) {
        selectedCaprin = caprin;
        isEdit = true;
        showModal = true;
    }

    function handleView(caprin: CaprinResponse) {
        ...
        caprinsStore.filters.set(filters);        
        caprinsStore.setCaprin(caprin);
        goto(`/caprins/${caprin.id}`);
    }
    ...
    async function handleSubmit(event: CustomEvent) {
        ...
        try {
            if (isEdit && selectedCaprin) {
                await caprinsApi.updateCaprin(selectedCaprin.id, formData);
            } else {
                await caprinsApi.createCaprin(formData);
            }
            showModal = false;
            await loadData();
        } catch (error) {
            console.error("Failed to save caprin:", error);
        }
    }
...
<div class="min-h-screen bg-gradient-to-br from-green-50 to-blue-50">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-4 mb-8">
            <div class="bg-white rounded-xl shadow-md p-5 hover:shadow-lg transition-shadow duration-300 border-l-4 border-blue-500">
                <div class="flex items-center justify-between">
                    <div>
                        <p class="text-sm text-gray-500">Total caprins</p>
            ...
            <div class="bg-white rounded-xl shadow-md p-5 hover:shadow-lg transition-shadow duration-300 border-l-4 border-purple-500">
                <div class="flex items-center justify-between">
                    <div>
                        <p class="text-sm text-gray-500">Boucs / Chèvres</p>
        ...
        {:else if filteredCaprins.length === 0}
            <div class="bg-white rounded-xl shadow-md p-12 text-center">
                <div class="text-7xl mb-4">🐐</div>
                <h3 class="text-xl font-medium text-gray-900 mb-2">Aucun caprin trouvé</h3>
                <p class="text-gray-500 mb-6">Aucun caprin ne correspond à vos critères de recherche.</p>
                <Button on:click={handleAdd} variant="primary">➕ Ajouter un caprin</Button>
            </div>
        {:else}
                ...
                <div class="overflow-x-auto">
                    <table class="min-w-full divide-y divide-gray-200">
                        ...
                        <tbody class="bg-white divide-y divide-gray-100">
                            {#each filteredCaprins as caprin}
                                <tr class="hover:bg-gray-50 transition-colors duration-150">
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        {#if caprin.photo_url}
                                            <img
                                                src={getFullPhotoUrl(caprin.photo_url)}
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <span class="font-semibold text-gray-900">{caprin.identification}</span>
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-600">{caprin.race || "-"}</td>
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        {#if caprin.sexe === "male"}
                                            <span class="inline-flex items-center gap-1 px-2 py-1 rounded-full text-xs font-medium bg-blue-100 text-blue-800">♂ Bouc</span>
                                        {:else if caprin.sexe === "femelle"}
                                            <span class="inline-flex items-center gap-1 px-2 py-1 rounded-full text-xs font-medium bg-pink-100 text-pink-800">♀ Chèvre</span>
                                        {:else}
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-600">{caprin.age_mois || "-"} mois</td>
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <div class="flex gap-1 flex-wrap">
                                            {#if caprin.production_viande}
                                                <span class="inline-flex items-center gap-1 px-2 py-0.5 rounded text-xs font-medium bg-red-100 text-red-800">🥩 Viande</span>
                                            {/if}
                                            {#if caprin.production_reproduction}
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap text-right">
                                        <div class="flex justify-end gap-2">
                                            {#if caprin.statut === 'vivant' || caprin.statut === 'transfere'}
                                                <button
                                                    on:click={() => handleEdit(caprin)}
    ...
    <!-- Modal formulaire -->
    <Modal open={showModal} title={isEdit ? "✏️ Modifier le caprin" : "➕ Ajouter un caprin"} on:close={() => (showModal = false)} size="lg">
        <CaprinForm
            formData={selectedCaprin ? {
                type_espece: 'caprin',
                race: selectedCaprin.race,
                sexe: selectedCaprin.sexe,
                date_naissance: selectedCaprin.date_naissance || null,
                date_arrivee: selectedCaprin.date_arrivee,
                provenance: selectedCaprin.provenance,
                prix_achat: selectedCaprin.prix_achat || undefined,
                enclos_id: selectedCaprin.enclos_id || 0,
                statut: selectedCaprin.statut,
                production_viande: selectedCaprin.production_viande,
                production_reproduction: selectedCaprin.production_reproduction,
                notes: selectedCaprin.notes || undefined,
                photo_url: selectedCaprin.photo_url || null
            } : {
                type_espece: "caprin",
                race: "",
                sexe: "male",
