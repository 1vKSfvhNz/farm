<!-- lib/components/forms/OvinForm.svelte -->
...
<script lang="ts">
    ...
    let localEnclosName = formData.enclos_id || "";
    ...
    let localProductionLaine = formData.production_laine;
    let localQualiteLaine = formData.qualite_laine || "";
    let localNotes = formData.notes || "";

    const sexeOptions = [
        { value: "male", label: "Mâle (Bélier)" },
        { value: "femelle", label: "Femelle (Brebis)" },
    ...
    const qualiteLaineOptions = [
        { value: "extra_fine", label: "Extra fine" },
        { value: "fine", label: "Fine" },
        { value: "moyenne", label: "Moyenne" },
        { value: "grossiere", label: "Grossière" },
    ];
    ...
    function validate(): boolean {
        ...
        if (!localEnclosName) errors.enclos_id = "L'enclos est requis";
        if (localProductionLaine && !localQualiteLaine) {
            errors.qualite_laine = "La qualité de laine est requise pour la production de laine";
        }
        ...
        if (validate()) {
            ...
            if (localStatut === "decede" && isEdit) {
                const confirmSubmit = confirm(
                    "⚠️ ATTENTION : Ce ovin sera marqué comme DÉCÉDÉ.\n\n" +
            ...
            const submitData: Record<string, any> = {
                ...
                enclos_id: localEnclosName,
                statut: localStatut,
                production_viande: localProductionViande,
                production_reproduction: localProductionReproduction,
                production_laine: localProductionLaine,
                qualite_laine: localQualiteLaine || null,
...
<form on:submit={handleSubmit} class="space-y-6">
    ...
    <div class="border rounded-lg p-4 bg-gray-50">
        <h4 class="text-md font-medium text-gray-900 mb-3">📷 Photo de l'ovin</h4>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
        <Input
            ...
            required
            error={errors.race}
            placeholder="Ex: Merino, Suffolk, Dorset..."
        />
        <Select
            label="Sexe"
        ...
        {#if !isEdit}
            <Input
                ...
                inputType="number"
                placeholder="0"
                hint="Poids de l'ovin à l'arrivée"
            />
        {/if}
    ...
    <div class="border-t border-gray-200 pt-4">
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <label class="flex items-center gap-2">
                <input type="checkbox" bind:checked={localProductionLaine} class="rounded" />
                <span class="text-sm text-gray-700">🧶 Production laine</span>
            </label>
            <label class="flex items-center gap-2">
    ...
    <!-- Qualité de laine (visible uniquement si production laine est active) -->
    {#if localProductionLaine}
        <div class="border-t border-gray-200 pt-4 bg-blue-50 rounded-lg p-4">
            <h4 class="text-md font-medium text-gray-900 mb-3">🧶 Détails production de laine</h4>
            <Select
                label="Qualité de laine"
                bind:value={localQualiteLaine}
                options={qualiteLaineOptions}
                required
                error={errors.qualite_laine}
