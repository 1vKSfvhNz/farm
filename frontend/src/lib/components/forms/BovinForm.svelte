<!-- lib/components/forms/BovinForm.svelte -->
...
<script lang="ts">
    ...
    let localProductionLaitiere = formData.production_laitiere;
    let localProductionViande = formData.production_viande;
    let localProductionReproduction = formData.production_reproduction;
    let localLactationEnCours = formData.lactation_en_cours;
    let localProductionLait = formData.production_lait_quotidienne?.toString() || "";
        ...
        if (validate()) {
            ...
            if (localStatut === "decede" && isEdit) {
                const confirmSubmit = confirm(
                    "⚠️ ATTENTION : Ce bovin sera marqué comme DÉCÉDÉ.\n\n" +
            ...
            const submitData: Record<string, any> = {
                // Ajout de type_espece (obligatoire dans AnimalBase)
                type_espece: "bovin",
                race: localRace,
                sexe: localSexe,
                ...
                statut: localStatut,
                production_laitiere: localProductionLaitiere,
                production_viande: localProductionViande,
                production_reproduction: localProductionReproduction,
                lactation_en_cours: localLactationEnCours,
                production_lait_quotidienne: localProductionLait ? parseFloat(localProductionLait) : null,
...
<form on:submit={handleSubmit} class="space-y-6">
    ...
    <div class="border rounded-lg p-4 bg-gray-50">
        <h4 class="text-md font-medium text-gray-900 mb-3">📷 Photo du bovin</h4>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
        <Input
            ...
            required
            error={errors.race}
            placeholder="Ex: Limousine, Prim'Holstein..."
        />
        <Select
            label="Sexe"
        ...
        {#if !isEdit}
            <Input
                ...
                inputType="number"
                placeholder="0"
                hint="Poids du bovin à l'arrivée"
            />
        {/if}
    ...
    <div class="border-t border-gray-200 pt-4">
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
            <label class="flex items-center gap-2">
                <input type="checkbox" bind:checked={localProductionLaitiere} class="rounded" />
                <span class="text-sm text-gray-700">🥛 Production laitière</span>
            </label>
            <label class="flex items-center gap-2">
    ...
    {#if localProductionLaitiere}
        <div class="border-t border-gray-200 pt-4 bg-blue-50 rounded-lg p-4">
            <h4 class="text-md font-medium text-gray-900 mb-3">🥛 Détails production laitière</h4>
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <label class="flex items-center gap-2">
                    <input type="checkbox" bind:checked={localLactationEnCours} class="rounded" />
                    <span class="text-sm text-gray-700">Lactation en cours</span>
                </label>
                <Input
                    label="Production lait quotidienne (L)"
                    bind:value={localProductionLait}
                    inputType="number"
                    placeholder="0"
                />
            </div>
